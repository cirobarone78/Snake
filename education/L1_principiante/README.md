# L1 — Principiante · *Investor 101*

> Alfabetizzazione di base sui mercati finanziari. Nessuna formula, nessun
> prerequisito. Se non hai mai avuto un conto su un exchange o un broker,
> questo è il livello giusto.

## Capitoli

1. [Cos'è un asset, una borsa, un broker](./01_asset_borsa_broker.md) ✅
2. [Tipi di ordine: market, limit, stop](./02_tipi_di_ordine.md) ✅
3. [Come si legge un grafico di prezzo (candele, volume)](./03_lettura_grafico.md) ✅
4. [Fee, spread, slippage spiegati senza matematica](./04_fee_spread_slippage.md) ✅
5. [Portafoglio e diversificazione](./05_portafoglio_diversificazione.md) ✅
6. [DCA (Dollar Cost Averaging) — pro e contro](./06_dca_dollar_cost_averaging.md) ✅
7. [Volatilità e drawdown: cosa significano davvero](./07_volatilita_drawdown.md) ✅
8. *(pianificato)* Custodia: cold/hot wallet vs custodia su exchange
9. *(pianificato)* Fiscalità essenziale (concettuale, non consulenza)
10. *(pianificato)* Cosa NON è il trading: le promesse a cui non credere

## Filosofia di L1

- Spiegare **una cosa alla volta**, senza saltare passaggi
- Mai usare gergo senza spiegarlo
- Esempi concreti con asset reali (BTC, ETH) dove possibile
- Onestà sui rischi: chi promette guadagni facili sta mentendo
