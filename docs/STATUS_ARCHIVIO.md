# STATUS — archivio storico

> **Cronologia congelata di `STATUS.md`** al 2026-08-24 (commit `f07dd3d`, subito
> dopo il merge della PR #52), prima della compressione prevista da
> **WP0** di [`docs/PIANO_SVILUPPO.md`](./PIANO_SVILUPPO.md).
>
> `STATUS.md` è stato ridotto a fotografia dello stato corrente (< 200 righe).
> **Tutto ciò che è stato tolto da lì è qui sotto, verbatim**: nessuna riga della
> cronaca di sessione è andata persa. Da qui in avanti, la regola operativa è:
> `STATUS.md` descrive **l'oggi**, questo file conserva **il come ci siamo
> arrivati**. Quando `STATUS.md` torna a gonfiarsi, la cronaca vecchia si sposta
> in coda a questo archivio (in ordine cronologico inverso, come già è).
>
> ⚠️ **Documento storico**: i numeri, i branch e i "prossimi step" citati qui
> valgono alla data della sessione che li ha scritti e **non sono aggiornati**.
> Per lo stato reale leggere sempre [`../STATUS.md`](../STATUS.md).

---

## Contenuto archiviato

- Fotografia della fase corrente al 2026-08-24 (pre-compressione)
...
## Fotografia congelata (era la testa di STATUS.md)
...
### 2026-08-24 — Sessione WP0: riconciliazione, fotografia, direzione di prodotto

Primo work package di `docs/PIANO_SVILUPPO.md`, eseguito su `main` post-merge
della PR #52 (`f07dd3d`). **Nessun codice toccato**: solo Markdown.

- **`STATUS.md` compresso** da 1497 a 156 righe e riscritto come *fotografia*
  (branch/commit, workflow, risultati empirici consolidati, blocchi, prossime
  attività) invece che come diario. Tutta la cronaca precedente è finita in
  questo file, verbatim: verificato riga per riga che l'unica cosa non riportata
  fossero il titolo e il blockquote di intestazione, sostituiti qui sopra.
- **Crescita git misurata** su storia completa (il clone di sessione era shallow,
  `git fetch --unshallow` per avere numeri veri): 812 commit di cui 676 (83%)
  automatici, pack 1,17 GiB, 7 727 MiB di blob non compressi, di cui
  **7 532 MiB (97,5%) sono le 479 copie di `data/news_history/news.parquet`**.
  Tabella in `STATUS.md` e in ADR-032; è il contesto che l'ADR-033 (WP1) deve
  citare.
- **ADR-032 registrata** (`Accepted`): il primo prodotto predittivo dichiarato
  del progetto è il ranking cross-sectional di ETF settoriali con probabilità
  calibrate vs SPY. Include D1–D7, le ipotesi H1–H3 e la barra di adozione,
  scritte prima di qualunque backtest.
- **`ROADMAP.md`**: nuova **Fase 9 — Ranking ETF probabilistico** con i WP come
  checkbox. Collisione di numerazione risolta: il "Live trading", che era Fase 9,
  è diventato **Fase 10** — stessi gate di ADR-004, non riaperto.
- **`README.md`**: chiarito in testa che la repo non è un gioco Snake (il nome è
  un residuo, ADR-001), stato riallineato al 2026-08-24, rimando al piano,
  conteggio test aggiornato (era fermo a "67/67 al 2026-05-29").

**Fuori perimetro, annotato e non toccato** (regola §0.2 del piano):

- Il `.gitignore` esclude `*.parquet` e riammette gli storici con glob di
  cartella (`!data/news_history/*.parquet` e simili): le partizioni mensili di
  WP1, se restano in quelle cartelle, sono già coperte. WP1 lo verifichi comunque
  con `git check-ignore` prima del primo commit (lezione ADR-029).
- `dca.yml` è stato mergiato con la PR #52 ma **non ha ancora girato**: il primo
  run va osservato.
- Il piano chiedeva la tabella di crescita "dentro l'ADR-033", che però è un
  deliverable di WP1: è stata messa in ADR-032 e in `STATUS.md`, pronta da citare.
